import { jasmineAdvanced } from './hibrid-testing-framework';
...
jasmineAdvanced.describe("Extensión de Pruebas: Búsqueda Binaria", () => {
  
  // 1. Property-Based Testing (Simulación manual de invariantes)
  jasmineAdvanced.it("Property-Based: El resultado debe ser un índice válido o -1 para 100 arreglos aleatorios ordenados", () => {
    for (let i = 0; i < 100; i++) {
      ...
      if (resultIndex !== -1) {
        jasmineAdvanced.expect(randomArr[resultIndex]).toBe(target); // Invariante: El valor en el índice debe ser el objetivo
      }
    }
  });

  // 2. Mutation Testing (Simulación de mutante de operador relacional de frontera de bucle)
  jasmineAdvanced.it("Mutation Testing: Debería fallar si cambiamos el operador de frontera (Simulación de Mutante left < right)", () => {
    ...
    jasmineAdvanced.expect(result).toBe(4); // Si da 4, el mutante fue exitosamente MATADO por la prueba.
  });

  // 3. Contract Testing (Prueba de Contrato de Precondición y Postcondición)
  jasmineAdvanced.it("Contract Testing: El contrato exige que la entrada esté ordenada. Postcondición garantiza consistencia.", () => {
    ...
    jasmineAdvanced.expect(index).toBe(2);
  });

  // Pruebas Automatizadas Basadas en Tipos (Sección 1 integrada)
  jasmineAdvanced.autoGenTypeTests(binarySearch, 'array');
});

// Ejecutar el motor
jasmineAdvanced.runSuites();